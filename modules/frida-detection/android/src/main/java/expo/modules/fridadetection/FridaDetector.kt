...
object FridaDetector {
    ...
    private val MAP_HINTS = arrayOf(
        "frida", "frida-agent", "frida-agent.so", "frida-gadget",
        "frida:rpc", "gum-js-loop", "linjector", "re.frida.server",
        "libfrida", "LIBFRIDA", "/data/local/tmp/frida",
    )
    private val PROCESS_HINTS = arrayOf(
        "frida-server", "frida-helper", "frida-agent",
        "re.frida.server", "linjector", "frida",
    )
    private val THREAD_HINTS = arrayOf(
        "gum-js-loop", "gmain", "frida", "pool-frida",
    )
    ...
    private fun scoreAnonRxFromMaps(maps: String): Int {
        ...
        for (line in maps.lineSequence()) {
            ...
            if (!perms.contains("r-x")) continue
    ...
    private fun scorePorts(): Int {
        if (isAnyFridaPortConnectable()) return SCORE_PORT_HIT
        if (isAnyFridaPortListeningInProcNet()) return SCORE_PORT_HIT
        return 0
    }
    ...
    private fun isAnyFridaPortListeningInProcNet(): Boolean {
        for (path in arrayOf("/proc/net/tcp", "/proc/net/tcp6")) {
            if (isFridaPortListeningInProcNetFile(File(path))) return true
        }
        return false
    }
    ...
    private fun isFridaPortListeningInProcNetFile(file: File): Boolean {
        ...
        return try {
            file.readLines().drop(1).any { line ->
                ...
                if (parts[3] != PROC_NET_LISTEN_STATE) return@any false
    ...
    private fun scoreProcesses(): Int {
        ...
        for (entry in entries) {
            ...
            try {
                ...
                val cmdline = raw.toString(Charsets.UTF_8).replace('\u0000', ' ').trim()
